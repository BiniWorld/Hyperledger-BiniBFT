...
const (
	...
	// ConsensusTypeBiniBFT identifies the BiniBFT-based consensus implementation.
	ConsensusTypeBiniBFT = "binibft"
...
func NewOrdererGroup(conf *genesisconfig.Orderer, channelCapabilities map[string]bool) (*cb.ConfigGroup, error) {
	...
	case ConsensusTypeBiniBFT:
		consenterProtos, err := consenterProtosFromConfig(conf.ConsenterMapping)
		if err != nil {
			return nil, errors.Errorf("cannot load consenter config for orderer type %s: %s", ConsensusTypeBiniBFT, err)
		}
		addValue(ordererGroup, channelconfig.OrderersValue(consenterProtos), channelconfig.AdminsPolicyKey)
		// Marshal BiniBFT configuration
		if consensusMetadata, err = genesisconfig.MarshalBiniBFTOptions(conf.BiniBFT); err != nil {
			return nil, errors.Errorf("consenter options read failed with error %s for orderer type %s", err, ConsensusTypeBiniBFT)
		}
		// Overwrite policy manually by computing it from the consenters
		policies.EncodeBFTBlockVerificationPolicy(consenterProtos, ordererGroup)
	default:
