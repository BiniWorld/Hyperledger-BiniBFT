...
// Consenter implementation of the BiniBFT based consenter
...
// New creates Consenter of type binibft
func New(
	...
	logger := flogging.MustGetLogger("orderer.consensus.binibft")
...
func (c *Consenter) ReceiverByChain(channelID string) MessageReceiver {
	...
	if biniBFTChain, isBiniBFT := cs.Chain.(*BFTChain); isBiniBFT {
		return biniBFTChain
	}
	c.Logger.Warningf("Chain %s is of type %v and not binibft.Chain", channelID, reflect.TypeOf(cs.Chain))
	return nil
}
...
func (c *Consenter) HandleChain(support consensus.ConsenterSupport, metadata *cb.Metadata) (consensus.Chain, error) {
	...
	configOptions, err := createBiniBFTConfig(support.SharedConfig())
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed parsing binibft configuration")
	}
	c.Logger.Debugf("BiniBFT config: %+v", config)
	...
	chain, err := NewChain(
		...
		(uint64)(selfID),
		config,
		path.Join(c.WALBaseDir, "binibft", support.ChannelID()),
		c.ClusterDialer,
	...
	if err != nil {
		return nil, errors.Wrap(err, "failed creating a new BiniBFTChain")
	}
